# Kubernetes Resources for deployment of Integrator and Broker profiles of WSO2 Enterprise Integrator with Analytics
...
`KUBERNETES_HOME/integrator-broker-analytics/test/rdbms/mysql` for deploying the product databases, using MySQL in Kubernetes. However, this approach of product database deployment is
...
* For using these Kubernetes resources,
    ...
    ```
    kubectl create configmap mysql-dbscripts --from-file=<KUBERNETES_HOME>/integrator-broker-analytics/test/confs/rdbms/mysql/dbscripts/
    ```

    Then, create a Kubernetes service (accessible only within the Kubernetes cluster) and followed by the MySQL Kubernetes deployment, as follows:
    
    ```
    kubectl create -f <KUBERNETES_HOME>/integrator-broker-analytics/test/rdbms/mysql/mysql-service.yaml
    kubectl create -f <KUBERNETES_HOME>/integrator-broker-analytics/test/rdbms/mysql/mysql-deployment.yaml
